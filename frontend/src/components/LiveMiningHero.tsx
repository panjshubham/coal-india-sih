...
export default function LiveMiningHero() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans border-b border-slate-200 dark:border-slate-800">
      {/* Background Video Stream - using a placeholder image gradient if video is missing */}
      <div className="absolute inset-0 bg-white dark:bg-slate-900 w-full h-full object-cover scale-105 filter brightness-95 contrast-105">
      ...
      <div className="absolute right-8 top-1/4 z-20 hidden lg:flex flex-col gap-3 font-mono text-xs">
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md p-3.5 rounded-lg border border-cyan-500/40 shadow-xl text-cyan-300 w-64">
          <div className="flex justify-between items-center pb-1 border-b border-slate-300 dark:border-slate-700/60 font-bold">
            <span>BUCKET WHEEL #BWE-01</span>
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
          </div>
          <div className="mt-2 space-y-1 text-[11px] text-slate-700 dark:text-slate-300">
            <p>Rotation: <strong className="text-slate-900 dark:text-white">{telemetry.bweRpm}</strong></p>
            <p>Bench Cut Depth: <strong className="text-slate-900 dark:text-white">12.4 Meters</strong></p>
            <p>Vibration Index: <strong className="text-emerald-700 dark:text-emerald-400">Nominal (0.12g)</strong></p>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md p-3.5 rounded-lg border border-amber-500/40 shadow-xl text-amber-300 w-64">
          <div className="flex justify-between items-center pb-1 border-b border-slate-300 dark:border-slate-700/60 font-bold">
            <span>LHD / SDL UNDERGROUND FLEET</span>
            <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
          </div>
          <div className="mt-2 space-y-1 text-[11px] text-slate-700 dark:text-slate-300">
            <p>LHD Unit 4: <strong className="text-slate-900 dark:text-white">{telemetry.lhdStatus}</strong></p>
            <p>SDL Unit 2: <strong className="text-slate-900 dark:text-white">{telemetry.sdlStatus}</strong></p>
            <p>Air Methane Level: <strong className="text-emerald-700 dark:text-emerald-400">{telemetry.methane} (Safe)</strong></p>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-20 max-w-7xl mx-auto h-full flex flex-col justify-center px-8">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-200 dark:bg-amber-500/20 border border-amber-500/40 text-amber-700 dark:text-amber-400 font-mono text-xs w-fit mb-4 backdrop-blur">
          <span className="w-2 h-2 rounded-full bg-amber-400 animate-ping" />
          <span>LIVE CONTINUOUS PIT EXTRACTION TELEMETRY</span>
        </div>

        <h1 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white max-w-3xl leading-tight tracking-tight drop-shadow-lg">
          Automated Pit Governance & Continuous Mining Safety
        </h1>

        <p className="text-slate-800 dark:text-slate-200 text-lg max-w-2xl mt-4 leading-relaxed drop-shadow-md">
