  ...
  const getCustomIcon = (score: number, isActive: boolean) => {
    ...
    const htmlString = `
      <div class="relative w-6 h-6 rounded-full ${colorClass} flex items-center justify-center ${shadowClass} ring-2 ring-white/90 ${scaleClass} transition-transform">
        <span class="w-2 h-2 rounded-full bg-slate-50 dark:bg-slate-950"></span>
      ...
      <div className="map-wrapper w-screen h-screen relative overflow-hidden select-none">
        ...
        <header className="fixed top-0 left-0 right-0 h-10 z-[1000] px-6 flex items-center justify-between backdrop-blur-md bg-[#0E172A]/75 border-b border-white/[0.08] text-xs shadow-sm">
          ...
          <div className="hidden lg:flex items-center gap-6 font-mono text-[10px] text-slate-600 dark:text-slate-400">
            <div className="flex items-center gap-1.5">
              <span className="text-slate-700 dark:text-slate-500">MONITORED SITES:</span>
              <span className="text-slate-700 dark:text-slate-300 font-medium">{mines.length} MINES LIVE</span>
            </div>
          </div>
          <div className="flex items-center gap-4 font-mono text-[10px]">
            <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
              <span className="text-slate-700 dark:text-slate-300">{time}</span>
            </div>
          </div>
        </header>
              ...
              >
                <Popup className="custom-popup dark-popup">
                  <div className="p-2 font-sans">
                    <div className="flex items-center gap-1.5 mb-1.5 text-rose-700 dark:text-rose-400 font-bold text-xs">
                      <span className="material-symbols-outlined text-[14px]">warning</span>
                      High-Risk Hotspot
                    </div>
                    <p className="text-slate-700 dark:text-slate-300 text-[11px] m-0 leading-tight">
        ...
        <aside className="fixed top-14 left-6 z-[1000] w-72 backdrop-blur-md bg-[#0E172A]/85 border border-white/[0.08] rounded-xl shadow-2xl p-4 flex flex-col gap-3.5 transition-all max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar">
          <div className="flex items-center justify-between pb-2.5 border-b border-white/[0.06]">
            ...
            <span className="font-mono text-[10px] tracking-[0.08em] text-amber-700 dark:text-amber-400/90 uppercase px-2 py-0.5 rounded-md bg-amber-400/10 border border-amber-400/20 font-medium">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <span className="material-symbols-outlined absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-[16px] pointer-events-none">expand_more</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="grid grid-cols-4 gap-1 p-1 bg-[#080D1A]/90 border border-white/[0.06] rounded-lg text-center font-mono text-[10px]">
              ...
              <button 
                className={`py-1 rounded border font-medium transition-colors ${riskFilter === 'Critical' ? 'bg-status-rose/20 text-status-rose border-status-rose/30' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-status-rose'}`} 
                onClick={() => setRiskFilter('Critical')}
              >
                Critical
              </button>
              <button 
                className={`py-1 rounded border font-medium transition-colors ${riskFilter === 'Watch' ? 'bg-status-amber/20 text-status-amber border-status-amber/30' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-status-amber'}`} 
                onClick={() => setRiskFilter('Watch')}
              >
                Watch
              </button>
              <button 
                className={`py-1 rounded border font-medium transition-colors ${riskFilter === 'Compliant' ? 'bg-status-sage/20 text-status-sage border-status-sage/30' : 'border-transparent text-slate-600 dark:text-slate-400 hover:text-status-sage'}`} 
          ...
          <div className="pt-2 border-t border-white/[0.06] flex flex-col gap-1.5">
             ...
             <div className="flex flex-col gap-1.5 max-h-[250px] overflow-y-auto pr-1">
                 ...
                 return (
                   ...
                   >
                     <div className="flex flex-col gap-0.5 truncate pr-2">
                       <span className="font-sans text-xs text-slate-800 dark:text-slate-200 truncate">{mine.name}</span>
                       <span className="font-mono text-[9px] text-slate-700 dark:text-slate-500">{mine.subsidiary}</span>
                     </div>
               ...
               {filteredMines.length === 0 && (
                 <div className="text-center p-4 text-xs text-slate-700 dark:text-slate-500 italic">
        ...
        <div className="fixed top-14 right-6 z-[1000] backdrop-blur-md bg-[#0E172A]/85 border border-white/[0.08] px-4 py-2 rounded-full flex items-center gap-4 text-xs text-slate-700 dark:text-slate-300 shadow-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-sans text-[11px] text-slate-700 dark:text-slate-300 font-normal">Compliant <span className="font-mono text-[10px] text-slate-600 dark:text-slate-400">(&lt;45)</span></span>
          </div>
          <span className="w-px h-3 bg-white/[0.1]" />
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-status-amber shadow-[0_0_8px_#F59E0B]" />
            <span className="font-sans text-[11px] text-slate-700 dark:text-slate-300 font-normal">Watch <span className="font-mono text-[10px] text-slate-600 dark:text-slate-400">(45–75)</span></span>
          </div>
          <span className="w-px h-3 bg-white/[0.1]" />
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-status-rose shadow-[0_0_8px_#F87171]" />
            <span className="font-sans text-[11px] text-slate-700 dark:text-slate-300 font-medium">Critical <span className="font-mono text-[10px] text-rose-300">(&gt;75)</span></span>
          </div>
        </div>
