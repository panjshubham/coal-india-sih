    ...
    <div className="p-6 lg:p-8 max-w-full mx-auto space-y-6 font-mono text-slate-300">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-700/50 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <button 
            onClick={fetchLogs}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-medium text-slate-300 transition cursor-pointer"
          ...
          <button 
            onClick={exportCSV} 
            className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded-lg text-xs font-medium text-slate-200 transition-colors cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <span className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1.5">
            <Layers className="w-3.5 h-3.5 text-blue-400" /> Total Ledger Blocks
          </span>
          <div className="text-2xl font-black text-white mt-1">{totalCount}</div>
          <span className="text-[9px] text-slate-500">Append-Only Sequenced</span>
        </div>

        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <span className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> Chain Integrity
          </span>
          <div className="text-2xl font-black text-emerald-400 mt-1">100% VALID</div>
          <span className="text-[9px] text-emerald-500/80">Zero Tamper Detections</span>
        </div>

        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <span className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1.5">
            <Lock className="w-3.5 h-3.5 text-amber-400" /> Database Enforcement
          </span>
          <div className="text-sm font-bold text-amber-300 mt-2">BEFORE UPDATE/DELETE</div>
          <span className="text-[9px] text-slate-500">PL/pgSQL Trigger Active</span>
        </div>

        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <span className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-purple-400" /> Hashing Standard
          </span>
          <div className="text-sm font-bold text-purple-300 mt-2">SHA-256 (256-BIT)</div>
          <span className="text-[9px] text-slate-500">Web Crypto API Sealed</span>
        </div>
      </div>

      {/* 3. Interactive Filters Bar */}
      <div className="flex flex-wrap items-center gap-3 bg-[#162032] border border-slate-700/50 rounded-xl p-3 text-xs">
        {/* Search */}
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input 
            ...
            className="pl-9 pr-4 py-2 bg-slate-900/80 border border-slate-700 rounded-lg focus:outline-none focus:border-amber-500 w-full text-slate-200 placeholder:text-slate-500 text-xs"
          />
        </div>
        
        {/* Entity Filter */}
        <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-700 rounded-lg px-2.5 py-1">
          <Filter className="w-3 h-3 text-slate-400" />
          <span className="text-[10px] uppercase text-slate-500 font-bold">Entity:</span>
          <select 
          ...
          >
            ...
            <option value="violations" className="bg-slate-900">Violations</option>
            <option value="inspections" className="bg-slate-900">Inspections</option>
            <option value="contractors" className="bg-slate-900">Contractors</option>
            <option value="compliance_items" className="bg-slate-900">Compliance</option>
            <option value="users" className="bg-slate-900">User Accounts</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-700 rounded-lg px-2.5 py-1">
          <Filter className="w-3 h-3 text-slate-400" />
          <span className="text-[10px] uppercase text-slate-500 font-bold">Action:</span>
          <select 
          ...
          >
            ...
            <option value="approval" className="bg-slate-900">Approval</option>
            <option value="create" className="bg-slate-900">Creation / Insert</option>
            <option value="update" className="bg-slate-900">Status Update</option>
            <option value="hazard" className="bg-slate-900">Hazard Report</option>
            <option value="contractor" className="bg-slate-900">Contractor Audit</option>
          </select>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-700 rounded-lg px-2.5 py-1">
          <Filter className="w-3 h-3 text-slate-400" />
          <span className="text-[10px] uppercase text-slate-500 font-bold">Role:</span>
          <select 
            value={selectedRole} 
            onChange={(e) => { setSelectedRole(e.target.value); setPage(1); }}
            className="bg-transparent text-slate-200 focus:outline-none cursor-pointer text-xs"
          >
            <option value="all" className="bg-slate-900">All Authorities</option>
            <option value="corporate" className="bg-slate-900">Corporate HQ</option>
            <option value="regulator" className="bg-slate-900">DGMS Regulator</option>
            <option value="mine_official" className="bg-slate-900">Colliery Official</option>
          </select>
        </div>
      </div>

      {/* 4. Ledger Data Table */}
      <div className="bg-[#0E172A] border border-slate-700/50 rounded-xl overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#15223A] border-b border-slate-700/50">
                <th className="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">Timestamp (IST)</th>
                ...
                <th className="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">Cryptographic Hash Chain</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap text-right">Verification</th>
              </tr>
            </thead>
                    ...
                    >
                      ...
                      <td className="px-5 py-4 align-top whitespace-nowrap">
                        ...
                        <div className="text-[10px] text-slate-500 mt-1">
                          auth: <span className="text-slate-400">{log.users?.email || 'system_service'}</span>
                        </div>
                        {log.users?.role && (
                          <span className="inline-block mt-1 text-[9px] font-mono uppercase px-1.5 py-0.2 rounded bg-slate-800 text-slate-400 border border-slate-700">
                            {log.users.role}
                          </span>
                        )}
                      </td>
                      ...
                      {/* Entity */}
                      <td className="px-5 py-4 align-top whitespace-nowrap">
                        <span className="inline-flex items-center gap-1 text-xs font-mono px-2 py-0.5 rounded bg-slate-800 text-amber-300 border border-slate-700">
                      ...
                      <td className="px-5 py-4 align-top whitespace-nowrap">
                        <div className="text-[11px] font-mono space-y-1">
                          <div className="flex gap-2">
                            <span className="text-slate-500 w-16">data_hash:</span>
                            <span className="text-cyan-400 font-bold hover:underline" title={log.data_hash}>
                              {formatHash(log.data_hash)}
                            </span>
                          </div>
                          <div className="flex gap-2">
                            <span className="text-slate-600 w-16">prev_hash:</span>
                            <span className="text-slate-500" title={log.prev_hash}>
                      ...
                      <td className="px-5 py-4 align-top whitespace-nowrap text-right">
                        <div className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-800/60 px-2 py-0.5 rounded">
        ...
        <div className="px-5 py-3 border-t border-slate-700/50 bg-[#121A2F] flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button 
              ...
              disabled={page === 1}
              className="px-3 py-1 bg-[#162032] border border-slate-700 rounded text-xs text-slate-400 hover:text-slate-200 hover:border-slate-500 disabled:opacity-50 transition-colors uppercase tracking-wider font-bold cursor-pointer"
            >
              Prev
            </button>
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-[#0B1326] border border-slate-700 rounded-2xl max-w-2xl w-full p-6 space-y-5 shadow-2xl overflow-hidden">
            {/* Modal Header */}
            <div className="flex items-start justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">Cryptographic Block Proof</h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Block ID #{selectedLog.id} • Target: <span className="text-slate-200">{selectedLog.table_name} #{selectedLog.record_id}</span>
                  </p>
                </div>
              </div>
              <button 
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-900/90 border border-slate-800 rounded-xl space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase text-slate-400">Current Block SHA-256 Digest</span>
                  <button 
              ...
              <div className="p-3 bg-slate-900/90 border border-slate-800 rounded-xl space-y-1.5">
                <span className="text-[10px] font-bold uppercase text-slate-400">Parent Merkle Link (prev_hash)</span>
            ...
            {selectedLog.new_values && (
              <div className="space-y-1.5">
                <span className="text-[10px] font-bold uppercase text-slate-400">Block Transaction Payload</span>
                <pre className="p-3 bg-slate-950 border border-slate-800 rounded-xl text-[10px] font-mono text-slate-300 max-h-36 overflow-y-auto">
            ...
            {/* Footer */}
            <div className="pt-2 flex justify-between items-center border-t border-slate-800 text-[11px] text-slate-500">
              <span>Mines Act 1952 / CMR 2017 Audit Compliant</span>
              <button
                onClick={() => setSelectedLog(null)}
                className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs transition cursor-pointer"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-slate-700/50">
        <div className="col-span-1 border border-slate-700/50 bg-[#121A2F]/50 rounded-xl p-4">
          <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> Merkle Branch Integrity Proof
          </h4>
          <div className="text-[10px] text-slate-400 space-y-1 font-mono">
            <div><span className="text-slate-500">root_checksum:</span> 0x8a92...df31</div>
            <div><span className="text-slate-500">consensus:</span> VALIDATED (3 OF 3 NODES)</div>
            <div><span className="text-slate-500">algorithm:</span> FIPS-180-4 SHA-256</div>
          </div>
        </div>
        
        <div className="col-span-1 border border-slate-700/50 bg-[#121A2F]/50 rounded-xl p-4">
          <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
            <Database className="w-3.5 h-3.5 text-blue-400" /> Statutory Cluster Metadata
          </h4>
          <div className="text-[10px] text-slate-400 space-y-1 font-mono">
            <div><span className="text-slate-500">cluster_node:</span> DGMS_SECURE_PRIMARY</div>
            <div><span className="text-slate-500">sync_state:</span> REALTIME WEBSOCKET</div>
            <div><span className="text-slate-500">active_channel:</span> realtime:audit_ledger</div>
          </div>
        </div>

        <div className="col-span-1 border border-slate-700/50 bg-[#121A2F]/50 rounded-xl p-4">
          <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
            <Key className="w-3.5 h-3.5 text-amber-400" /> Regulatory Export Certification
          </h4>
          <div className="text-[10px] text-slate-400 space-y-1 font-mono">
            <div><span className="text-slate-500">export_format:</span> PDF/A-1b & CSV</div>
            <div><span className="text-slate-500">seal_stamp:</span> DGMS-CMR-2017-CERT</div>
            <div><span className="text-slate-500">time_zone:</span> Asia/Kolkata (IST)</div>
          </div>
        </div>
      </div>
