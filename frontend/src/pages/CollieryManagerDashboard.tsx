    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        {[
          ...
          <div key={card.label} className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col shadow-sm">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
            {SHIFT_DATA.map((s) => (
              <div key={s.shift} className="p-4 flex flex-col gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900/40 transition">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col shadow-sm">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/60 flex-1">
            {CONTRACTOR_CHECKINS.map((c) => (
              <div key={c.name} className="p-3.5 flex flex-col gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900/40 transition">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col shadow-sm">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/60 flex-1 overflow-y-auto max-h-80">
            ...
            {violations.map(v => (
              <div key={v.id} className="p-3.5 flex flex-col gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900/40 transition relative overflow-hidden">
      ...
      {/* 4. Compliance Table */}
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl flex flex-col mt-6 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-2">
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-mono">
              ...
              {complianceItems.map(item => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700 dark:bg-slate-900/40 transition">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => { setShowHazardForm(false); resetForm(); }} className="text-slate-700 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-100 dark:text-white transition">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {['low', 'medium', 'high', 'critical'].map(sev => (
                    <button
                      ...
                      className={`py-1.5 rounded border text-[10px] font-bold uppercase transition-all font-mono ${formData.severity === sev ? severityColor[sev] : 'text-slate-600 dark:text-slate-500 border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/50 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-slate-900'}`}
                    >
                      {sev}
                    </button>
                  ))}
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex gap-3">
              <button
                ...
                className="flex-1 py-2 rounded border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-400 text-xs font-bold font-mono hover:bg-slate-100 dark:hover:text-slate-100 dark:text-white transition"
              >
