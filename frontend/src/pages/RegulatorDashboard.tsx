    ...
    <div className="min-h-screen bg-[#070D18] text-slate-100 font-sans p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <p className="text-xs font-mono text-slate-400 uppercase">{t('reg_metric_monitored_mines', 'Monitored Mines')}</p>
          <div className="flex items-baseline justify-between mt-1">
            <h3 className="text-3xl font-black text-white">18</h3>
            <span className="text-xs text-emerald-400 font-mono">{t('reg_metric_active_100', '100% Active')}</span>
          </div>
        </div>
        ...
        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          ...
          <div className="flex items-baseline justify-between mt-1">
            ...
            <span className="text-xs text-slate-400 font-mono">{t('reg_metric_action_flagged', 'Action Flagged')}</span>
          </div>
        </div>

        <div className="p-4 bg-[#0B1326] border border-slate-800 rounded-xl">
          <p className="text-xs font-mono text-slate-400 uppercase">{t('reg_metric_risk_score', 'Composite Risk Score')}</p>
          <div className="flex items-baseline justify-between mt-1">
            <h3 className="text-3xl font-black text-cyan-400">29 <span className="text-sm font-normal text-slate-500">/ 100</span></h3>
            <span className="text-[10px] text-slate-400 font-mono">{t('reg_metric_xgboost', 'XGBoost Weighted')}</span>
          </div>
        </div>
      </div>
      ...
      {verifiedHash && (
        <div className="mt-4 p-3 bg-emerald-950/80 border border-emerald-500 rounded-lg text-emerald-300 text-xs font-mono flex items-center justify-between transition-all">
          ...
          <span className="text-[10px] text-emerald-400/80">{t('reg_tamper_proof', 'Tamper-Proof Proof-of-State')}</span>
        </div>
      )}
        ...
        <div className="bg-[#0B1326] border border-slate-800 rounded-xl p-4 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <span className="text-[10px] font-mono text-slate-400">{t('reg_xgboost_shap', 'XGBoost + SHAP')}</span>
            </div>
      ...
      <div className="mt-6 p-3 bg-[#0B1326] border border-slate-800 rounded-lg flex items-center justify-between text-xs font-mono text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-300">{t('reg_live_audit_stream', 'LIVE DGMS AUDIT STREAM:')}</span>
          <span>{t('reg_audit_log_example', '[10:14:02 IST] Karo Spl Bench #4 inspection anchored to ledger (0x7f8a...c3d1)')}</span>
        </div>
        <span className="text-emerald-400 hidden sm:inline">{t('reg_chain_validated', 'CHAIN VALIDATED')}</span>
      </div>
