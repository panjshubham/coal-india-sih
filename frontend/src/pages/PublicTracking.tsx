...
export default function PublicTracking() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#070D18] text-slate-100 font-sans selection:bg-amber-500/30 flex flex-col">
      
      {/* Top Banner */}
      <div className="text-[10px] font-mono py-1.5 px-6 flex justify-between items-center tracking-widest bg-emerald-950/40 border-b border-emerald-300 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400/80">
      ...
      <header className="px-6 py-4 flex items-center justify-between border-b border-white/5 bg-slate-50 dark:bg-[#0B1326]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight text-slate-900 dark:text-white">CoalGuard</h1>
            <p className="text-[10px] font-mono text-slate-600 dark:text-slate-400 uppercase tracking-widest">National Compliance Monitor</p>
          </div>
        </div>
      ...
      <main className="max-w-6xl w-full mx-auto px-6 py-12 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-[#0B1326] border border-white/5 relative overflow-hidden group hover:border-amber-400 dark:border-amber-500/30 transition-colors">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <MapIcon className="w-16 h-16 text-amber-700 dark:text-amber-400" />
            </div>
            <p className="text-xs font-mono text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Monitored Pits</p>
            <div className="text-5xl font-black text-slate-900 dark:text-white">
              {loading ? '-' : stats.activeMines}
            </div>
          </div>
          
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-[#0B1326] border border-emerald-300 dark:border-emerald-500/20 relative overflow-hidden group shadow-[0_0_30px_rgba(16,185,129,0.05)]">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <ShieldCheck className="w-16 h-16 text-emerald-700 dark:text-emerald-400" />
            </div>
            <p className="text-xs font-mono text-emerald-700 dark:text-emerald-400/80 uppercase tracking-wider mb-2">National Compliance</p>
            <div className="text-5xl font-black text-emerald-700 dark:text-emerald-400">
              {loading ? '-' : `${stats.complianceRate}%`}
            </div>
          </div>
          
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-[#0B1326] border border-white/5 relative overflow-hidden group hover:border-blue-400 dark:border-blue-500/30 transition-colors">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <Users className="w-16 h-16 text-blue-700 dark:text-blue-400" />
            </div>
            <p className="text-xs font-mono text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Registered Contractors</p>
            <div className="text-5xl font-black text-slate-900 dark:text-white">
              {loading ? '-' : stats.contractors}
            </div>
          </div>
          
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-[#0B1326] border border-white/5 relative overflow-hidden group hover:border-violet-500/30 transition-colors">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <Activity className="w-16 h-16 text-violet-700 dark:text-violet-400" />
            </div>
            <p className="text-xs font-mono text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Annual Inspections</p>
            <div className="text-5xl font-black text-slate-900 dark:text-white">
        ...
        <div className="p-8 rounded-2xl bg-gradient-to-br from-blue-950/40 to-slate-900 border border-blue-300 dark:border-blue-500/20 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button className="flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-slate-900 dark:text-white font-semibold transition-colors shrink-0">
      ...
      {/* Footer */}
      <footer className="border-t border-white/5 py-8 text-center text-xs font-mono text-slate-700 dark:text-slate-500 mt-auto">
