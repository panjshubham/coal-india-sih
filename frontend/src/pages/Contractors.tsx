    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6 w-full font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">Manage third-party operators and their compliance history.</p>
        </div>
        
        <div className="flex items-center gap-3">
          <button
            onClick={() => { setGatePassModalOpen(true); setVerificationResult(null); }}
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 hover:to-teal-600 text-slate-900 dark:text-white font-bold text-xs rounded-lg transition shadow-[0_0_15px_rgba(16,185,129,0.3)] border border-emerald-400 dark:border-emerald-500/30"
          >
          ...
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-700 dark:text-slate-500" />
            <input 
              type="text" 
              placeholder="Search name, license..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-9 pr-4 py-2 bg-white dark:bg-[#070D18] border border-slate-200 dark:border-slate-800 rounded-lg shadow-inner text-sm text-slate-700 dark:text-slate-300 focus:ring-1 focus:ring-amber-500 focus:border-amber-500 w-full sm:w-64"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 dark:bg-[#0B1326] border border-slate-200 dark:border-slate-800 shadow-xl rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white dark:bg-[#070D18] border-b border-slate-200 dark:border-slate-800">
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap">Contractor Name</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap">License & Status</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap text-center">Safety Score</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap text-center">Open Violations</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap text-center">Audit</th>
                <th className="px-5 py-3 text-[10px] font-bold text-slate-700 dark:text-slate-500 uppercase tracking-widest text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-700 dark:text-slate-500">Loading contractors...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-700 dark:text-slate-500">No contractors found.</td>
                </tr>
              ) : (
                filtered.map((c) => {
                    ...
                    <tr key={c.id} className="hover:bg-white/5 transition-colors group cursor-pointer relative" onClick={() => window.location.href = `/contractors/${c.id}`}>
                      <td className="px-5 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                            {c.name}
                            {status.isExpired && (
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-red-200 dark:bg-red-500/20 text-red-700 dark:text-red-400 border border-red-400 dark:border-red-500/30">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex flex-col items-start gap-1">
                          <span className="text-sm text-slate-600 dark:text-slate-400 font-mono">{c.license_no}</span>
                          <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded border ${
                            status.isExpired ? 'bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-400 dark:border-red-500/30' : 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-400 dark:border-emerald-500/30'
                          }`}>
                      ...
                      <td className="px-5 py-4 text-center">
                        {openViolations > 0 ? (
                          <span className="inline-flex items-center gap-1 text-sm font-bold text-amber-500 bg-amber-100 dark:bg-amber-500/10 border border-amber-400 dark:border-amber-500/30 px-2.5 py-1 rounded">
                            <ShieldAlert className="w-4 h-4" /> {openViolations}
                          </span>
                        ) : (
                          <span className="text-slate-700 dark:text-slate-500 text-sm font-medium">None</span>
                        )}
                      </td>
                      <td className="px-5 py-4 text-center">
                        <button
                          ...
                          className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-emerald-200 dark:bg-emerald-500/20 text-slate-600 dark:text-slate-400 hover:text-emerald-700 dark:text-emerald-400 transition-colors border border-slate-300 dark:border-slate-700"
                        >
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link 
                          to={`/contractors/${c.id}`} 
                          className="inline-flex items-center justify-center px-3 py-1.5 bg-white dark:bg-[#070D18] border border-slate-300 dark:border-slate-700 rounded text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-800 transition-colors shadow-sm"
      ...
      {auditModalOpen && selectedContractor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white dark:bg-slate-900/80 backdrop-blur-sm" onClick={() => setAuditModalOpen(false)}>
          <div className="bg-slate-50 dark:bg-[#0B1326] border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-[#070D18]">
              <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                <Fingerprint className="w-5 h-5 text-emerald-500" />
                Tamper-Evident Ledger: {selectedContractor.name}
              </h3>
              <button onClick={() => setAuditModalOpen(false)} className="text-slate-700 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              <p className="text-xs text-slate-600 dark:text-slate-400 mb-4 bg-white dark:bg-[#070D18] p-3 rounded border border-slate-200 dark:border-slate-800 shadow-inner">
              ...
              {auditLoading ? (
                <div className="text-center text-sm text-slate-700 dark:text-slate-500 py-4">Verifying blockchain ledger...</div>
              ) : auditLogs.length === 0 ? (
                <div className="text-center text-sm text-slate-700 dark:text-slate-500 py-4">No audit events found for this contractor.</div>
              ) : (
                <div className="space-y-3">
                  {auditLogs.map((log) => (
                    <div key={log.id} className="border border-slate-200 dark:border-slate-800 rounded-lg p-3 bg-white dark:bg-[#070D18]">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">{log.action}</span>
                        <span className="text-xs text-slate-700 dark:text-slate-500">{new Date(log.created_at).toLocaleString()}</span>
                      </div>
                      <div className="bg-slate-50 dark:bg-[#0B1326] border border-slate-200 dark:border-slate-800 rounded p-2 overflow-x-auto">
                        <div className="flex items-center gap-2 text-emerald-500 font-mono text-[10px]">
                          <ShieldCheck className="w-3 h-3 shrink-0" />
                          <span>SHA-256:</span>
                          <span className="tracking-wider text-emerald-700 dark:text-emerald-400">{log.data_hash}</span>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      {gatePassModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm overflow-y-auto" onClick={() => setGatePassModalOpen(false)}>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden my-8" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/80">
              ...
              <button onClick={() => setGatePassModalOpen(false)} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white p-1 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white font-mono"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-slate-600 dark:text-slate-400 block mb-1">
                        ...
                        <span className="text-[10px] text-slate-700 dark:text-slate-500 block">(1 Year Validity)</span>
                      </label>
                      <input
                        type="date"
                    ...
                    <div>
                      <label className="text-xs text-slate-600 dark:text-slate-400 block mb-1">
                        ...
                        <span className="text-[10px] text-slate-700 dark:text-slate-500 block">(5 Year Validity)</span>
                      </label>
                      <input
                        type="date"
                ...
                {/* Simulated QR Badge */}
                <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center text-center space-y-2">
                  ...
                  <span className="text-[9px] text-slate-700 dark:text-slate-500 uppercase tracking-widest">
                ...
                <div className={`p-4 rounded-xl border ${
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-xs font-mono font-bold uppercase ${
                      verificationResult.gate_interlock === 'BARRIER_OPEN'
                        ? 'bg-emerald-900/60 text-emerald-300 border border-emerald-400 dark:border-emerald-500/30'
                        : 'bg-red-900/60 text-red-300 border border-red-400 dark:border-red-500/30'
                    }`}>
                      {verificationResult.gate_interlock}
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 dark:bg-slate-950/60 p-3 rounded-lg border border-white/5">
                    <div>
                      <span className="text-slate-600 dark:text-slate-400 block">VTC Refresher Status:</span>
                      <strong className={verificationResult.vtc_compliance.status === 'VALID' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}>
                        {verificationResult.vtc_compliance.status} ({verificationResult.vtc_compliance.days_until_refresher} days left)
                      </strong>
                    </div>
                    <div>
                      <span className="text-slate-600 dark:text-slate-400 block">PME Medical Status:</span>
                      <strong className={verificationResult.pme_compliance.status === 'FIT' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}>
                  ...
                  <ul className="text-xs space-y-1 list-disc list-inside text-slate-700 dark:text-slate-300">
                    {verificationResult.findings.map((f: string, i: number) => (
                      <li key={i} className={f.includes('LAPSED') || f.includes('EXPIRED') ? 'text-red-700 dark:text-red-400 font-bold' : ''}>{f}</li>
                    ))}
                  </ul>

                  {verificationResult.access_status !== 'ACCESS_GRANTED' && (
                    <div className="pt-2 border-t border-red-400 dark:border-red-500/30 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition ${
                          incidentLogged ? 'bg-emerald-800 text-emerald-200' : 'bg-red-600 hover:bg-red-500 text-slate-900 dark:text-white'
                        }`}
                      >
