...
function DocumentCameraScanner({
    ...
    // Ensure all media tracks are cleanly stopped on unmount
    useEffect(() => {
        return () => {
            stopCamera();
        };
    ...
    const startCamera = async (facing: 'user' | 'environment' = facingMode) => {
        ...
        // Soft constraints with graceful fallback - never use rigid { exact: ... } parameters
                ...
                <div className="p-4 bg-slate-100 dark:bg-slate-800/80 border border-blue-400 dark:border-blue-500/30 rounded-xl space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={handleSpeak}
                                className={`flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-lg border transition-all ${speaking ? 'bg-blue-500/30 border-blue-500/60 text-blue-200' : 'bg-white/5 border-white/10 text-slate-700 dark:text-slate-300 hover:bg-white/10'
                                    }`}
                            >
        ...
        <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
                <button
                    ...
                    className={`flex items-center gap-2.5 px-6 py-3 rounded-xl text-sm font-semibold transition-all border shadow-lg ${recording
                ...
                <div className="p-4 bg-slate-100 dark:bg-slate-800/80 border border-rose-500/30 rounded-xl space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={handleSpeak}
                                className={`flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-lg border transition-all ${speaking ? 'bg-rose-500/30 border-rose-500/60 text-rose-200' : 'bg-white/5 border-white/10 text-slate-700 dark:text-slate-300 hover:bg-white/10'
                                    }`}
                            >
                                {speaking ? <VolumeX className="w-3.5 h-3.5" /> : <Volume2 className="w-3.5 h-3.5" />}
                                <span>{speaking ? 'Stop' : 'Read Aloud (TTS)'}</span>
...
function PPEPanel() {
    ...
    const analyzeImagePixels = (img: HTMLImageElement): {
        ...
        // Head region: upper 30% of person height
        const headY1 = personYMin;
        const headY2 = Math.min(height - 1, personYMin + Math.round(personH * 0.30));
        const headX1 = Math.max(0, personXMin + Math.round(personW * 0.10));
        const headX2 = Math.min(width - 1, personXMax - Math.round(personW * 0.10));
        ...
        const finalHeadBox = (hasHardHat && helmXMax > helmXMin)
            ...
            : {
                ...
                xmax: headX2 / width, ymax: headY2 / height
            };
        ...
        const finalTorsoBox = (hasVest && vestXMax > vestXMin)
            ...
            : {
                xmin: torsoX1 / width, ymin: torsoY1 / height,
                xmax: torsoX2 / width, ymax: torsoY2 / height
            };
    ...
    return (
        <div className="space-y-4">
            ...
            {result && (
                <div
                    className={`p-5 rounded-2xl border transition-all ${result.compliance_status === 'COMPLIANT'
                ...
                >
                    <div className="flex items-start justify-between flex-wrap gap-2 mb-3">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <span
                                    className={`font-black tracking-wider text-base ${result.compliance_status === 'COMPLIANT'
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                                {result.missing_ppe.map((item: string) => {
                                    ...
                                    return (
                                        <span
                                            key={item}
                                            className={`px-3 py-1 rounded-full text-xs font-bold font-mono border flex items-center gap-1 ${isBorderline
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/10 flex-wrap gap-3">
                        <div className="text-[11px] font-mono text-slate-600 dark:text-slate-400 flex items-center gap-3">
                            <span>Helmet Coverage: <strong className="text-slate-800 dark:text-slate-200">{result.pixel_metrics?.helmet_color_coverage_pct}%</strong> (min 6%)</span>
                            <span>Vest Coverage: <strong className="text-slate-800 dark:text-slate-200">{result.pixel_metrics?.vest_color_coverage_pct}%</strong> (min 8%)</span>
                        </div>
                        ...
                        {result.compliance_status !== 'COMPLIANT' && (
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${ticketCreated
        ...
        <div className="space-y-4">
            ...
            {preview && (
                <div className="p-4 bg-white dark:bg-slate-900/60 border border-white/10 rounded-xl space-y-3">
                    ...
                    {result && (
                        <div className={`p-4 rounded-xl border ${result.compliance_status === 'COMPLIANT'
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                {result.compliance_status !== 'COMPLIANT' && (
                                    <button
                                        ...
                                        disabled={ticketCreated}
                                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition ${ticketCreated
